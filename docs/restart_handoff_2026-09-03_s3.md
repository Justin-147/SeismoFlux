...
更新时间：2026-09-03 14:04（北京时间；归因持续运行，已有结果的离线回放已生成）。
...
原归因仍为唯一S3后台进程PID **39156**，状态`predicting_nulls`。14:03:43已保存
**168/4000块（4.2%）**，失败0；时间两折完整复本 **24/200、9/200**，空间两折 **0/200、0/200**，
仍按每折先时间后空间执行。14:03:51进程CPU约2.63%、内存3.63GiB、整机CPU5%，BelowNormal，
stderr为空。不要重启、改模型或提前评分；10个归因绑定实现文件本轮再次核对未变。
