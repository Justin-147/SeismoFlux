...
更新时间：2026-09-03 15:07（北京时间；归因持续运行，震序/地区分层及对照图已完成）。
...
原归因仍为唯一S3后台进程PID **39156**，状态`predicting_nulls`。15:06:08已保存
**403/4000块（10.075%）**，失败0；时间两折完整复本 **57/200、23/200**，空间两折 **0/200、0/200**，
仍按每折先时间后空间执行。15:06:29进程CPU约2.93%、内存3.62GiB、整机CPU21.5%，BelowNormal，
