...
当前正在`loading_parallel_frozen_inputs`，还不能声称8个计算子进程已经开始；加载通过才生成
只读共享上下文并启动worker。**不要另起协调器，也不要退回旧null_runner恢复。** 后续先核对
PID28084、上述日志、manifest.worker_pids与子进程树，看到8个worker和新预测块后才关闭切换事项。
