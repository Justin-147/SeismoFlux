...
更新时间：2026-09-03 14:20（北京时间；归因持续运行，科研准则与心跳去重已核对生效）。
状态：`S3A_NULL_PREDICTIONS_RUNNING_REPLAY_CREATED`。
...
原归因仍为唯一S3后台进程PID **39156**，状态`predicting_nulls`。14:18:14已保存
**225/4000块（5.625%）**，失败0；时间两折完整复本 **32/200、13/200**，空间两折 **0/200、0/200**，
仍按每折先时间后空间执行。14:19:00进程CPU约3.73%、内存3.80GiB、整机CPU24.5%，BelowNormal，
stderr为空；以上为当时采样，不是后续占用保证。不要重启、改模型或提前评分；本轮未改任何预测实现。

用户最高科研准则已在唯一蓝图1.22、AGENTS、汇报记忆及现有ACTIVE心跳中完整生效，
本轮只核对，不增设第二套规则、不重复索取本次聚合公开授权、不重跑已解决事项。
已完成且已报告的问题，只有新证据、复发或用户主动询问才重开。
...
下一科学工作：保留同一200+200归因任务，完成全部预测后统一评分；可并行补已有诊断的震序等权、
区域分层及原定不确定性汇总，再做完整S3-A复审。不重复准备、训练、主评分、震例账本或回放生成。
分层汇总的纯组件`src/seismoflux/multitask_s3/strata_summary.py`与其合成测试
`tests/test_multitask_s3_strata_summary.py`已落本机但尚未提交/集成，不能称为真实分层结果已完成。
它保留调用方权重，不在窗口/地区内重新归一化，返回配对增失及震序/起报聚类的命中和总权重。
下一执行者需先完成薄runner、验收与白名单提交推送，再读既有事件账本汇总；不得重训或增加目标。
震序成员数从同一合法历史和既定首震锚点定义取得；地区沿用静态网格映射，分组只解释适用范围，
不是新空间留出验证，也不是用区域一致性否定局部改善。
本轮回放展示到此完成，不继续围绕美化/浏览器工具投入，除非用户报告实际阅读问题。
当前贡献是既有效果的可查看、可追溯展示，尚无新预测增益或最终归因结论；科学路线无需改变。
