...
- 历史状态：`superseded`
- 被替代原因：`0.4.2` 把公开审计摘要误作推理 payload，且 `.jsonl` ledger 合同与单映射 CAS
  原语不一致；当前协议见 `0.4.3` / `v0.3.3-kde-anomaly-increment-protocol`
- 证据保留：以下 `v0.3.2` 身份、本地验证和复审记录均为历史事实，保持原文，不构成当前执行授权
...
> 本节是 `v0.3.2` 的历史待办，已被 `v0.3.3` 外部门替代，不得再执行或补记为当前事实。
