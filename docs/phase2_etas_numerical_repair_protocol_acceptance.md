...
- R3 勘误标签：`v0.2.2-background-etas-repair-protocol-r3`（保持不可变，但平台合同 NO-GO）
- 当前 R4 勘误标签：`v0.2.2-background-etas-repair-protocol-r4`（尚未创建）
...
- R4 勘误精确基线：R3 annotated tag object `bd77f35d3d676893c6e83f385c810c89965f6257`，peel 到提交 `8ccbf12dd971ee9e683b544e336c76b2a254eb60`
- 状态：`R4_NOT_ACCEPTED_NO_COMMIT`（只表示提交、推送和远端 annotated tag 尚未完成）。第一轮三路、第二轮差异和第三轮文档均曾明确 NO-GO；第四轮三路清零后 strict mypy 又发现固定四元组机械类型错误，在第五轮前最终受控回归仍未授权。该错误只用运行时恒等 cast 修复；第五轮同五哈希平台、差异、文档三路再次全部 `P0=0/P1=0/P2=0`。同快照 R4 专属 `72 passed, 69 deselected in 22.90s`、四项边界 `4 passed in 8.19s`、完整协议 `141 passed in 91.96s`、关联 `173 passed in 107.06s`、严格 YAML/Ruff/format/strict mypy/diff 全部通过；唯一最终受控非目标全量为 `1329 passed, 2 skipped in 423.18s`、无 failure/error。历史入文前限定 `72 passed, 69 deselected in 24.98s` 和边界 `4 passed in 8.05s` 继续保留，回填后时长只记 Git-ignored 实时交接。本次回填后的边界/静态与独立文档范围复核清零即授权单一五文件提交；远端 R4 tag 核验前 Stage 2R-A 继续暂停。

本次验收只冻结目标盲修复设计、25 个既有优化初值、输入/执行封印、调用回执、适配边界和停止条件。它不实施修复、不生成真实 fit-only 输入包、不运行真实 ETAS 资格拟合，也不创建新的阶段 4 执行修订。

## R4 Windows bootstrap 勘误边界（当前，尚未验收）

R3 已经提交并由 annotated tag 冻结，不能移动、覆盖或删除；其 schema、hash、身份链和 scientific closure 仍是 R4 比较基线。但冻结后执行的原生只读探针证明：R3 把严格 launcher `\\?\Volume{GUID}\suffix` 直接转换为 `\??\Volume{GUID}\suffix`，同时在 root absolute `NtCreateFile` 设置 `OBJ_DONT_REPARSE`，会因入口 Volume GUID symbolic link 稳定失败为 `STATUS_REPARSE_POINT_ENCOUNTERED (0xC000050B)`。所以 R3 不能执行 selected Windows profile，结论为不可变 NO-GO；R3 先前的限定、独立复审和全量数字只保留为历史工程证据。

R4 只修复该首个 absolute root bootstrap：

- CLI 仍只接受严格 canonical `\\?\Volume{GUID}\suffix`，不新增 drive/UNC/alias 输入；每个 initial install 或 fresh checkpoint session 都重新拆出 GUID root 和 exact suffix；
- authority 是对 explicit global link `\GLOBAL??\Volume{GUID}` 执行 `NtOpenSymbolicLinkObject(SYMBOLIC_LINK_QUERY)` + `NtQuerySymbolicLinkObject` 得到 `T0`；link-open OA 固定 `RootDirectory=NULL`、exact global ObjectName 无尾斜杠、`Attributes=0`（无 case-insensitive / no-reparse）、Security 两指针为 NULL、输入 `UNICODE_STRING Length==MaximumLength==exact UTF-16LE bytes` 且无隐含 NUL。query 输出必须满足偶数 `ReturnedLength == TargetName.Length + 2`、`MaximumLength >= ReturnedLength`；`T0` 只取 `Buffer[0:TargetName.Length]`，其后至 ReturnedLength 的两字节必须恰为 UTF-16 NUL 且不得进入 `T0`。该 `Attributes=0` 仅适用于 symbolic-link metadata open，绝不是 root bootstrap fallback。`QueryDosDeviceW("Volume{GUID}")` 只作独立 effective-map crosscheck：256 WCHAR 起步，仅 `ERROR_INSUFFICIENT_BUFFER` 可倍增到 32768 WCHAR/最多 8 次；按 returned WCHAR count 读取完整双-NUL `MULTI_SZ`，第一项必须非空，禁止 `buffer.value`/首 NUL 截断；只比较/选择第一项，允许完整解析后续旧映射但绝不选择第二项；
- pre/post global query 的 target exact UTF-16 bytes 必须不变，pre/post effective-map 第一项都必须等于同一 `T0`；先行 `\??` 探针没有排除 process-local shadow。explicit-global ad-hoc 非变更强化探针已经观察到 global/effective byte equality 和 pre/post no-drift；同一窄义 primitive 已进入 Windows tracked permanent pytest，第一轮终审发现的 scope、launcher lifetime、raw `MULTI_SZ` 和失败脱敏缺口已经修订并在当前限定中执行通过，仍待新哈希独立复审；
- `T0` 必须是 exact UTF-16/NFC、无任何 C0 control（包括 NUL/CR/LF）、绝对 `\Device\...` 安全路径，component 非空且不是 `.`/`..`，无 `/`、重复或尾随 separator；拒绝 `\??`、`\DosDevices`、`\GLOBAL??`、UNC、MUP；不得假设或硬编码 `HarddiskVolume[数字]`；
- root ObjectName 精确为 `T0 + 原 suffix`，仍使用 `RootDirectory=NULL + OBJ_DONT_REPARSE`、无 `OBJ_CASE_INSENSITIVE`、R3 原 DesiredAccess/ShareAccess/disposition/options。禁止 alias open、bootstrap-only `Attributes=0` retry、把 `QueryDosDeviceW` 当 authority、hardcode/cache device name 或 Win32 path API fallback；
- root handle 保持存活并同时回核 `GetFinalPathNameByHandleW` 的 `VOLUME_NAME_GUID` 精确 launcher path、`VOLUME_NAME_NT` 精确 constructed path；`FileIdInfo` u64 VolumeSerial/FileId 只与 launcher fixture/认证目录链的同 API 值比较，`GetVolumeInformationByHandleW` DWORD serial 只与同一 fixture handle 的 DWORD 比较并用于 NTFS crosscheck，禁止两种 serial 跨 API 等同或截断；另回核 disk/nonremote、root nonreparse及 handle-relative opening `HEAD`/upstream/repair-code tag/protocol package blobs；随后才做 post-query；
- direct target 严格 session-local、nonpersistent，每个 fresh session 重派生，绝不进入 envelope、receipt、log、公开工件、cache 或 restart state；任一 resolver/query/grammar/drift/open/crosscheck 失败都在 evaluator 前进入 `invalid_execution`；
- root 以下所有 descendants 继续使用 R3 的 parent-handle-relative `OBJ_DONT_REPARSE` 合同。R4 不改变科学/Schema/公开路径/模型/网格/KDE/初值/objective/优化器/门限/outer-SHA/外锚/失败状态或 publication 合同。

第一轮平台终审还发现一个继承文字歧义：上层 generic create/write/flush/install/reopen failure action 曾一律写为 `invalid_execution`，而既有唯一 `install_failure_state_machine` 已把 install 成功至完整 post-install 验证之间冻结为 `indeterminate_after_install`。R4 只把 generic action 改为无条件委派这张既有 phase-specific state machine，不改变任何 phase、status、恢复权限或科学语义；该单叶澄清必须在 R3→R4 exact diff 中单独列出，不得借此放宽其他 failure-state 差异。

R4 验收顺序固定为：复跑已修正的 tracked native permanent pytest → YAML/文档/协议测试限定复验 → direct-device resolver 的 malformed/mapping-drift/fallback 永久反例 → 同一快照独立平台与差异终审清零 → 完整协议、关联回归与严格静态限定 → 唯一一次最终受控非目标全量 → 正式回填验收 → 单一允许文件集提交 → 推送分支 → 创建并推送 annotated R4 tag → 远端 peel 核验。首轮限定虽通过，但第一轮三路终审发现可共同假通过的状态漂移、`MULTI_SZ` 空项漏口、过宽 leaf allowlist、R3 alias 旧指令、失败状态歧义、root-binding/native scope 和日志脱敏缺口，已判 NO-GO。前三项修订和当前限定复验已经完成；下一步是对新五哈希从头三路复审。新快照复审清零前不得启动完整协议、关联或最终受控全量，后续步骤不得提前；复审全零后这些剩余回归均为 R4 验收必做项，不得选择性省略。

首轮 tracked 定向必须作为 R4 负证据而非 PASS 保留：结果为 `39 passed / 1 failed / 2 collection-setup errors`。两个 setup error 来自超长/孤代理负例的 pytest 自动参数 ID 编码；唯一 native 失败先后暴露两项真实合同错误：实际 `NtQuerySymbolicLinkObject` 为 `ReturnedLength=48`、`TargetName.Length=46`，证明 ReturnedLength 包含尾随两字节 UTF-16 NUL；同一卷的 `FILE_ID_INFO.VolumeSerialNumber=15908439068185257866`，而 `GetVolumeInformationByHandleW` serial DWORD 为 `3324045194`，证明二者不能跨 API 强制相等。该首轮原始 stdout/JUnit 未持久化，因此三个摘要类别不能相加后冒充稳定 node inventory；当前有效 inventory 只能由修正后完整收集与复跑给出。修正 query buffer、serial 逐 API 比较和安全测试 ID 后曾从头得到 R4 专属 `41 passed, 69 deselected in 6.21s`、`41 passed, 69 deselected in 6.36s` 和 `41 passed, 69 deselected in 7.00s`；四个边界守卫曾为 `4 passed in 8.10s`、`4 passed in 9.25s` 和 `4 passed in 9.04s`。第一轮三路独立终审已明确 NO-GO 并否决这些旧快照，状态继续为 `R4_NOT_ACCEPTED_NO_COMMIT`。

## R3 勘误边界（历史，保持不可变但 NO-GO）
...
- 可信根只能来自本地 supervisor 传入且恰好一次出现的 `--workspace-root`：它必须指向 opening seal 已核验相同 `HEAD`、upstream、远端 code tag 和协议包 blobs 的 worktree，绝对值禁止进入公开工件。R3 历史条款曾要求把严格 `\\?\Volume{GUID}\...` 转换为 `\??\Volume{GUID}\...` 的 `NtCreateFile` ObjectName；该规则已被原生探针证伪，只作为不可变 NO-GO 历史保留，在 R4 中禁止实现、fallback 或验收。R4 当前唯一可执行 bootstrap 是本文件“R4 Windows bootstrap 勘误边界”冻结的 explicit-global `T0 + 原 suffix` 规则，live root handle 身份最终仍必须等于 outer-SHA 已认证目录链的第 0 项；
...
- R3 冻结版本已分别增加 Windows FileId-only 与 POSIX inode-only 单字段替换，保持 sealed 13 字段精确不变，并验证旧 gate/fit/presence 三锚整体与逐锚拒绝、全部新锚通过；同时新增九级目录身份链/链 SHA、严格 Volume GUID grammar、raw FileId fixture 和六类 `NtCreateFile` 结构化调用矩阵。
- live-chain 执行闭包补齐前的中间快照曾为完整协议 `67 passed in 60.76s`、关联 `99 passed in 61.24s`；随后禁止 embedded-chain copy 的版本为完整协议 `70 passed in 67.63s`、关联 `102 passed in 67.52s`。外部 registry 初版虽达到定向 `7 passed, 63 deselected in 11.85s`、完整协议 `70 passed in 67.16s`、关联 `102 passed in 69.30s`，平台终审仍以 `P0=0/P1=1/P2=0` 指出 source 构造后、provider 绑定前可改变配置，因此同样只保留为被否决的工程证据。最终 construction-snapshot 版本让 unbound registry 在 factory 构造完成时立即冻结 snapshot，provider 先消费并比较后才绑定；新增 pre-bind path→raw 与 raw-scalar mutation 反例后的限定为定向 `7 passed, 63 deselected in 11.48s`、完整协议 `70 passed in 67.56s`、协议+固定网格+local-support manifest `102 passed in 70.99s`，均零失败；Ruff check/format、单文件 strict mypy、YAML parse 和 `git diff --check` 也通过。随后同一五文件快照的三路复审曾全部 `P0=0/P1=0/P2=0`，唯一最终受控全量也曾通过并据此提交/打标签；但这些检查没有执行所冻结的 native absolute-root primitive，后发现探针使该平台 PASS 失效。

### R3 冻结后的 Windows 原生负证据（当前 R4 起因）

只读 2×2 探针固定 `RootDirectory=NULL`、directory DesiredAccess `0x001201bf`、ShareAccess `0x3`、`FILE_OPEN`，交叉改变 `OBJECT_ATTRIBUTES.Attributes` 与 `FILE_OPEN_REPARSE_POINT`：`Attributes=0` 的两格均 `STATUS_SUCCESS/FILE_OPENED`，但这是协议禁止的属性降级；`OBJ_DONT_REPARSE` 的两格均为 `STATUS_REPARSE_POINT_ENCOUNTERED (0xC000050B)`、`Information=0`、Win32 `4395`。因此 R3 alias bootstrap 没有合规成功格，R3 保持 NO-GO。

精确 ignored 证据为：

- `data/interim/stage2/etas_numerical_repair/r3_bootstrap_probe/probe_ntcreatefile_bootstrap.py`：size `5031`，SHA-256 `0c188d843e18a9a0dc006506c6ff39e6d6f79587b835b4a0620a7e01c5795c1c`；
- 同目录 `probe_output_redacted.jsonl`：size `1160`，SHA-256 `3ceeafb1898b9265c35fc538df74a75bfa1978520d6ce371a450bf2222107acf`；
- 同目录 `README.md`：size `3074`，SHA-256 `13eef2706fe582a606ec95d5ece7ed57ceea8e000a953d70a44de3cde19ffe4b`。

三者均由 `git check-ignore -v` 命中 `.gitignore:25:data/interim/**`。先行探针只查进程有效 `\??` 映射，不能排除 shadow；direct target + 原 suffix + `OBJ_DONT_REPARSE` 的四格可行性观察只支持 R4 设计方向。后续 explicit-global ad-hoc 非变更强化探针成功观察到 global open/query、`QueryDosDeviceW` cardinality 1 且与 `T0` byte-equal、direct `FILE_OPENED`、GUID/NT path equal 和 pre/post no-drift；首轮 tracked native 定向把它固化执行并暴露两项协议机械预期错误，修正后 native 节点曾通过。第一轮独立终审进一步确认该 node 的窄义 alias/direct-open 原语是真调用而不是文档模拟，但指出它不能冒充 descendant handle-relative repository identity/九级认证链生产实现，且失败日志必须脱敏，因此旧限定快照仍被否决。
...
### R4 direct-device bootstrap 勘误复验（首轮已被否决；修订后限定通过，待复审）

| 项目 | R4 当前结果 |
|---|---|
| 精确基线 | R3 annotated tag object `bd77f35d3d676893c6e83f385c810c89965f6257`，peel 到 commit `8ccbf12dd971ee9e683b544e336c76b2a254eb60`；已核对 |
| YAML parse | 第五轮清零后以重复 key 拒绝 loader 从头运行：`2 passed in 5.20s`；核对 `protocol_version=0.2.2`、`protocol_revision=r4` 与全部 refs |
| R4 专属定向 | 固定四元组修复后的同快照从头运行 `.venv/Scripts/python.exe -B -m pytest -q tests/unit/test_background_etas_numerical_repair_protocol.py -k r4`：`72 passed, 69 deselected in 22.90s`；历史入文前结果 `72 passed, 69 deselected in 24.98s` 继续保留 |
| 既有协议/文档边界守卫 | 正式回填前同快照为 `4 passed in 8.19s`；历史入文前冻结结果 `4 passed in 8.05s` 继续保留。回填后四项边界必须再运行，真实时长只记 Git-ignored 实时交接而不反写 tracked 文档，避免自引用 |
| Resolver 永久反例 | 当前限定通过；完整 parser 拒绝 terminal 前空项、单/三 NUL、zero/over-capacity count，bounded driver 只允许 `ERROR_INSUFFICIENT_BUFFER` 扩容并拒绝其他错误/耗尽 |
| Explicit-global ad-hoc 与 tracked native | ad-hoc 先行成功；首轮 tracked `39 passed / 1 failed / 2 collection-setup errors` 保留为负观察。第五轮平台线 `P0=0/P1=0/P2=0`，证明唯一测试变化是运行时恒等 cast，config 与全部 native primitive 未削弱；同快照 72 项限定实际运行，Windows native 0 skip |
| R3→R4 exact diff | 第五轮差异线 `P0=0/P1=0/P2=0`；撤销 cast 精确恢复第四轮测试 SHA，22 个 expected overlay 赋值无 current 借值，180 个叶差异只落 7 个预期顶层，30/37 顶层对象直接相等，六个真实 mutation 与 12 个保护单元不变 |
| 静态检查 | 第五轮后全仓 Ruff 通过；format 为 `224 files already formatted`；strict mypy 为 `Success: no issues found in 216 source files`；`git diff --check` 通过 |
| 第一轮独立终审 | NO-GO：平台 `P0=0/P1=4/P2=3`；差异/不变性 `P0=0/P1=1/P2=1`；文档/证据 `P0=0/P1=2/P2=3`。修复后必须用新五哈希从头三路复审，旧计数不得沿用 |
| 第二轮独立复审 | 总结论仍 NO-GO：平台 `P0=0/P1=0/P2=0`、聚焦 `21 passed in 5.75s` 且 native 0 skip；文档/证据 `P0=0/P1=0/P2=0`；差异/不变性 `P0=0/P1=1/P2=0`，聚焦 `6 passed in 7.47s` 但发现 expected overlay 自借 current。修复后必须生成新五哈希并执行第三轮三路复审 |
| 第三轮独立复审 | 总结论仍 NO-GO：平台 `P0=0/P1=0/P2=0`、聚焦 `22 passed in 23.32s` 且 native 0 skip；差异/不变性 `P0=0/P1=0/P2=0`、聚焦 `8 passed in 18.35s`；文档/证据 `P0=0/P1=1/P2=1`，发现当前边界证据仍写 `9.02s` 而权威复验为 `8.05s`，另有“当前 66 项”旧措辞。修复后必须生成新五哈希并执行第四轮三路复审 |
| 第四轮独立复审 | 同一五哈希三路均 `P0=0/P1=0/P2=0`：文档线只读清零；平台 `22 passed in 39.69s`、native 0 skip；差异 `8 passed in 35.77s`、exact overlay/structured diff/真实 mutation 全闭合。随后 strict mypy 类型修复改变测试字节，因此该 GO 不跨哈希授权后续最终全量 |
| 第四轮后修复前回归 | 完整协议 `141 passed in 97.25s`；协议+固定网格+local-support manifest `173 passed in 97.54s`；严格 YAML `2 passed in 4.59s`；Ruff/format/diff 通过。strict mypy 的单一机械类型错误使本组未完成，全部只作修复前证据 |
| 第五轮独立复审 | 固定四元组修复后的同一五哈希三路均 `P0=0/P1=0/P2=0`；文档线清零；平台/差异线因各自 CPU 首样本超过 70% 未强跑聚焦节点，但机械证明唯一 cast 运行时恒等，且外层同快照 72 项限定已受控通过 |
| 第五轮后最终限定 | 完整协议 `141 passed in 91.96s`；协议+固定网格+local-support manifest `173 passed in 107.06s`；严格 YAML `2 passed in 5.20s`；Ruff/format/strict mypy/diff 全部通过 |
| 受控全量 | 唯一一次最终受控非目标全量：`1329 passed, 2 skipped in 423.18s`，`failures=0`、`errors=0`；CPU `60.95%/46.18%/55.93%`，BelowNormal、affinity `0x3f0000`、数值库单线程 |
| 最终 JUnit | `data/interim/protocol-r4-final-full-nontarget.junit.xml`；size `218812`；SHA-256 `f5224e0b6183fc78262d6aeb22ee0a62b7c29a993c24979c840cba3ecf741d5a`；suite `tests=1331/failures=0/errors=0/skipped=2` |
| 提交/推送/R4 annotated tag | 尚未执行；本次回填后的边界/静态与独立文档范围复核清零后授权单一五文件提交 |
| 当前结论 | 本地工程回归已通过；远端 R4 tag 核验前仍以 `R4_NOT_ACCEPTED_NO_COMMIT` 表示外层未冻结，Stage 2R-A、真实 fit 源、真实 ETAS、Stage 4 正式目标和 Stage 9 锁定测试继续阻断 |

### R3 three-grid local persistence 勘误复验（历史，后发现平台 NO-GO）

| 项目 | R3 结果 |
|---|---|
| 首轮独立终审 | FAIL：`P0=1/P1=4/P2=1`；旧 PASS 撤销，旧测试数字不再作为 R3 接受证据 |
| 第二轮独立终审 | FAIL：公式 `P0=0/P1=2/P2=1`；证据闭包 `P0=0/P1=2/P2=0`；平台 `P0=0/P1=3/P2=0`。当时仍为 NO-GO |
| 第三轮独立终审 | FAIL：语义闭包 `P0=1/P1=2/P2=1`；平台合同 `P0=1/P1=4/P2=1`。上一版 `36/68` 限定复验被否决，当前再修订仍为 NO-GO |
| 第四轮独立终审 | FAIL：语义闭包 `P0=0/P1=1/P2=1`；公式/差异 `P0=0/P1=0/P2=1`；平台合同 `P0=0/P1=3/P2=2`。`58/90` 限定复验被否决，当时仍为 NO-GO |
...
| R3 冻结目录链/平台/provenance 最终限定 | 关键反例 `33 passed, 34 deselected in 32.34s`；首次独立 live-chain/root/POSIX 定向 `7 passed, 63 deselected in 10.52s`；R2→R3 exact deep-diff 与平台核心定向 `2 passed, 68 deselected in 7.63s`；中间 live-chain 版本 `70 passed in 67.63s`、关联 `102 passed in 67.52s`；被 pre-bind P1 否决的 registry 初版为定向 `7 passed, 63 deselected in 11.85s`、完整协议 `70 passed in 67.16s`、关联 `102 passed in 69.30s`；冻结 construction-snapshot 版本定向 `7 passed, 63 deselected in 11.48s`、完整协议 `70 passed in 67.56s`、关联 `102 passed in 70.99s`，均零失败，但未执行 native alias bootstrap |
...
| R3 冻结前三路独立终审 | 同一最终五文件快照的语义、公式/R2 差异、Windows/POSIX 文本/合成平台三路均为 `P0=0/P1=0/P2=0`；五文件二次 hash 均与指定值一致，但后发现没有执行 native alias bootstrap primitive，故不能维持平台 PASS |
...
| R3 精确改动范围 | 仅修改配置、主协议、验收记录、重启交接和协议测试五个授权 tracked 文件；随后已由 R3 commit/tag 冻结且不得更改 |
| 冻结后 native bootstrap 探针 | FAIL / NO-GO：合规 `OBJ_DONT_REPARSE` 两格均 `STATUS_REPARSE_POINT_ENCOUNTERED`；`Attributes=0` 两格虽打开成功但为禁止 fallback。R3 不得放行实现 |

R3 修复、限定复验和最终全量没有打开、stat、hash、查询或检查真实 fit 源/真实 bundle，没有运行 ETAS 拟合、阶段 4 正式目标消费者或阶段 9 锁定测试。R3 的 construction-snapshot 快照曾完成限定、静态、三路文本/合成终审和唯一最终受控全量，并已提交、推送和打 annotated tag；更晚的只读 native probe 证明其 selected Windows bootstrap 不可执行，因此 R3 结论必须区分为“证据/工程闭包历史通过、平台 primitive NO-GO”，不能继续放行实现。R4 当前仍在协议验收前，禁止提交。
...
当前 R4 仍以 `R4_NOT_ACCEPTED_NO_COMMIT` 标记提交与远端冻结尚未完成；这不再表示本地工程回归失败。第一轮三路、第二轮差异和第三轮文档曾明确 NO-GO；第四轮后 mypy 固定四元组错误又使旧 GO 失效；修复后的第五轮同哈希三路已全部清零，完整协议/关联/严格静态从头通过，唯一最终受控全量 `1329 passed, 2 skipped` 且无 failure/error。本次回填后的四项文档边界、静态检查和独立文档/范围终审如果清零，本文件即授权把精确五个 tracked 文件作为单一提交推送，并创建、推送及核验 `v0.2.2-background-etas-repair-protocol-r4`；不得夹带 ignored JUnit/runner/probe 或主工作树 Stage 2R-A 草稿。原始、R1、R2、R3 annotated tag 全部保持不动；只有远端 R4 tag 精确 peel 到本次提交后，才允许恢复 1 ULP 修复代码子阶段。这不表示 ETAS 已稳定拟合，更不表示预测效果或科学门控成功；当前没有实际预测效果静态图或交互页。
