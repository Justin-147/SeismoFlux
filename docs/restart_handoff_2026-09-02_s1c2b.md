...
> 22:12已进入S2方法和输入复核；续接时同时读`docs/s2_static_data_research_start_2026-09-02.md`。
> 22:30起最新恢复入口改为`docs/restart_handoff_2026-09-02_s2a.md`，本文件保留S1闭合证据。
...
## 10. S2已启动方法与输入复核：2026-09-02 22:12

S1闭合文档提交`7164e3b8cd2b8af939f18e40337d6c89ac476295`已经推送并远端确认。现在转到
`docs/s2_static_data_research_start_2026-09-02.md`：已阅读国内断层/平滑地震率研究、国际断层邻近
组合与GEAR/WHEEL原始资料，已只读核对519段简化几何和7,215条可用精细迹线。

原地质条目均为2026收集快照、`historical_model_eligible=False`；不修改这个标记。按S0允许的
当前静态几何描述性回放继续科学检验，不冒称当年已知，不因此停工。首轮只用几何，禁止混入当前
离逝率、上次强震年和危险性评分；不需要新清洗系统或未审核的段—迹线属性关联。

下一步：冻结一个有经典依据、可直接跑的纯几何配对，内层目录参数与混合权重都不能看较晚验证
目标；独立复审后实现/运行。现在尚未冻结S2数值协议、没有S2训练或新成绩，不把资料准备当效果。
本节与S2启动文档已本地落盘，待S2首个协议形成时一同验收同步。心跳保持开启，S1不再计算。
