...
"""Synthetic-only S2-A scientific figures and offline controls."""
...
def renderer():
    path = Path(__file__).resolve().parents[2] / "scripts/render_multitask_s2a.py"
    spec = importlib.util.spec_from_file_location("s2a_renderer_synthetic", path)
...
def fixture_data(renderer):
        ...
        for band in bands:
            for period in range(3):
                ...
                for model in models:
                    ...
                    if model.startswith("S2A_") and "COARSE" not in model:
    ...
    planned = [[a, b, label] for a, b, label in renderer.PAIR_AXES]
    ...
    summary = {
        ...
        "scientific_role": "current_static_geometry_retrospective_development_not_historical_prospective",
    }
...
def test_all_axes_and_empty_periods_preserved_with_s2_labels(renderer, fixture_data):
    ...
    assert all(len(issue["forecasts"]) == 10 for issue in data["issues"])
...
def test_case_choice_uses_only_four_mixtures_and_real_catalog_reference(renderer, fixture_data):
    data = renderer._replay_data(*fixture_data[:5])
    selection = renderer._select_cases(data)
    assert selection == renderer._select_cases(data)
    assert selection["candidate_model_id"] == "S2A_SIMPLE_CATALOG_MIX"
    ...
    for row in altered["summary"]["curves"]:
        if row["model_id"].endswith("FAULT_ONLY"):
...
def test_render_local_boundary_and_snapshot_caveat(renderer, fixture_data, tmp_path):
    ...
    assert "S2-A：断层位置" in text and "2026 年收集快照" in text
    ...
    assert data["default_candidate"] == "S2A_SIMPLE_CATALOG_MIX"
...
vm.runInContext(`
if(reference.value!=="C2B_D0_MULTISCALE"||candidate.value!=="S2A_SIMPLE_CATALOG_MIX"||tolerance.value!=="0")throw Error("wrong S2 defaults");
let count=0;
...
"""
    ...
    assert "1000 axes passed" in result.stdout
