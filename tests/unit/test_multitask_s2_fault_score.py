...
from seismoflux.multitask_s2 import fault_score as scorer
...
def _grid() -> SpatialGrid:
    return SpatialGrid(
        grid_id="synthetic-s2a-not-real",
        cell_size_km=25.0,
...
def _reference_fixture() -> tuple[dict, dict, np.ndarray, STRtree]:
    ...
    near = scorer.projected_near_cells(tree, [100.0], [30.0])
...
def test_incomplete_four_fold_gate_precedes_every_target_or_old_score_read(
    ...
    module = ModuleType("seismoflux.multitask_s2.fault_predict")
...
def test_exactly_eleven_pairs_do_not_append_old_c2b_comparisons(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    planned = [[f"model-{i}", "reference", f"purpose-{i}"] for i in range(11)]
    captured = {}
...
def test_grid_copy_is_byte_preserving_and_old_s1_cannot_be_output(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="multitask_s2"):
